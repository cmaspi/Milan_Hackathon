...
class Condense(nn.Module):
  ...
  def calculate_loss(self, before, after, sen_pred, adv_pred, sen_gold):
    ...
    asp_norm_loss = torch.matmul(asp_norm, asp_norm.t()) - torch.eye(self.aspect_dim)
    ...
    sen_norm_loss = torch.matmul(sen_norm, sen_norm.t()) - torch.eye(self.sentiment_dim)
...
class Abstract(nn.Module):
  ...
  def forward(self, tokens, token_ids, token_mask,
    ...
    p_copy = torch.zeros(batch_size, output_len, self.vocab_size)
  ...
  def beam_search(self, tokens, token_ids, token_mask,
    ...
    return torch.Tensor(finished['input']) 
